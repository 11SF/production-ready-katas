...
เขียน:
1. `WriteRecord(w io.Writer, payload []byte) error` — encode และเขียน record
2. `ReadRecord(r io.Reader) ([]byte, error)` — อ่านและ decode record หนึ่ง record
3. `ReadAllRecords(path string) ([][]byte, error)` — อ่านทุก record จากไฟล์
