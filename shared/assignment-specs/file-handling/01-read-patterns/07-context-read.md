...
- **ใช้จริง:** สำหรับ network connection ใช้ `SetDeadline()` / `SetReadDeadline()` — สำหรับ regular file ต้องใช้ goroutine + channel pattern เพราะ `os.File` ไม่รองรับ context โดยตรง
- **ทำ manual เมื่อ:** FIFO, pipe, หรือ socket — ไม่มี stdlib ช่วย ต้องออกแบบ goroutine lifecycle เอง
- **kata สอนว่า:** regular file I/O blocking ไม่มี built-in cancellation — ต้องรู้ก่อนออกแบบ service ที่มี timeout requirement
